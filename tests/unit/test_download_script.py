...
def test_hf_url_sends_bearer_when_token_set(dl, monkeypatch):
    ...
    headers = dl._http_headers("https://huggingface.co/foo/resolve/main/x.bin")
...
def test_non_hf_url_never_gets_token(dl, monkeypatch):
    ...
    monkeypatch.setenv("HF_TOKEN", "tok-abc")
    headers = dl._http_headers("https://example.com/file.bin")
    assert "Authorization" not in headers
...
def test_subdomain_of_hf_gets_token(dl, monkeypatch):
    ...
    monkeypatch.setenv("HF_TOKEN", "tok-abc")
    headers = dl._http_headers("https://cdn-lfs.huggingface.co/blob.bin")
    assert headers.get("Authorization") == "Bearer tok-abc"
